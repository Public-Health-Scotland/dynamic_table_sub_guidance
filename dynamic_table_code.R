...
#v2
  ...
  # Read archive file to filter out old rows
  ...
  nurse_fam_meas_data <- nurse_fam_meas_data %>%
    ...
    summarise(.groups = "drop") %>%   # Probably you want summarise here
    mutate(value = "x") %>%
    pivot_wider(
  ...
  flextable_format <- function(data) {
    data %>%
      ...
      border(part = "all", border = fp_border_default(color = "#000000", width = 0.5)) %>%
      autofit()
  }
  ...
  # Insert the flextable into the document at the bookmark in landscape layout
  doc <- doc %>%
